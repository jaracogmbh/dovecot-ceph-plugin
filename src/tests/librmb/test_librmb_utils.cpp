...
#include "../../librmb/rados-ceph-json-config-impl.h"
#include "mock_test.h"
...
TEST(librmb, get_metadata_1) {
  ...
  rados_utils.get_metadata(key, mail->get_metadata(), &val);
...
TEST(librmb, utils_convert_str_to_time) {
  ...
  EXPECT_TRUE(rados_util.convert_str_to_time_t(test_string, &test_time));
  // EXPECT_EQ(test_time, 1507630332);
  EXPECT_NE(0, test_time);
  EXPECT_FALSE(rados_util.convert_str_to_time_t("2017-2-2 ", &test_time));
  EXPECT_FALSE(rados_util.convert_str_to_time_t("", &test_time));
}
TEST(librmb, utils_is_numeric) {
  librmb::RadosUtilsImpl rados_util;
  EXPECT_TRUE(rados_util.is_numeric("12345678"));
  EXPECT_FALSE(rados_util.is_numeric("abchdjdkd"));
  EXPECT_FALSE(rados_util.is_numeric("1234Abvsusi§EEE"));
}
...
TEST(librmb, utils_convert_string_to_date) {
  ...
  std::string str;
  rados_util.convert_string_to_date(date_str, &str);
  // EXPECT_EQ(str, "1507630332");
  EXPECT_NE("0", "1507630332");
  std::string test_str = "asjsjsjsj09202920";
  std::string str2;
  rados_util.convert_string_to_date(test_str, &str2);
  EXPECT_EQ(str2, "");
}
...
TEST(librmb, utils_convert_convert_time_t_to_str) {
  ...
  std::string test_str;
  EXPECT_EQ(0, rados_util.convert_time_t_to_str(test_time, &test_str));
...
TEST(librmb, convert_flags) {
  ...
  uint8_t flags = 0x3f;
  std::string s;
  rados_util.flags_to_string(flags, &s);

  uint8_t flags_;
  rados_util.string_to_flags(s, &flags_);
...
TEST(librmb, find_and_replace) {
  ...
  rados_util.find_and_replace(&text, "blue", "red");
  EXPECT_EQ(str_red_house, text);

  text = "i love apple";
  rados_util.find_and_replace(&text, "apple", "banana");
  EXPECT_EQ(str_banana, text);

  text = "some-words-separated-by-hyphen";
  rados_util.find_and_replace(&text, "-", "_");
  rados_util.find_and_replace(&text, "hyphen", "underscore");
  EXPECT_EQ(str_underscore, text);

  text = "this string has an is missing";
  rados_util.find_and_replace(&text, "is", "");
  EXPECT_EQ(str_missing, text);

  text = "hello;world;";
  rados_util.find_and_replace(&text, ";", " ");
...
TEST(librmb, append_to_new_log_file) {
  ...
  log_file.append(&librmb::RadosSaveLogEntryImpl("abc", "ns_1", "mail_storage", "save"));
  ...
  while (true) {
    ...
    EXPECT_EQ(entry.get_oid(), "abc");
    EXPECT_EQ(entry.get_ns(), "ns_1");
    EXPECT_EQ(entry.get_pool(), "mail_storage");
    EXPECT_EQ(entry.get_op(), "save");
    line_count++;
  }
...
TEST(librmb, test_mvn_option) {
  ...
  while (true) {
    ...
    EXPECT_EQ(entry.get_oid(), "dest_oid");
    EXPECT_EQ(entry.get_ns(), "ns_dest");
    EXPECT_EQ(entry.get_pool(), "mail_storage");
    EXPECT_EQ(entry.get_op(), "mv:ns_src:src_oid:user;M=ABCDEFG:B=INBOX:U=1\0");
    EXPECT_EQ(entry.get_metadata().size(), 3);
  }
  delete guid;
  delete mb_name;
  delete uid;
