...
TEST(librmb, split_write_operation) {
  ...
  obj.get_mail_buffer()->str(splitable_buffer);
  int buffer_length = obj.get_mail_buffer()->str().length();
...
TEST(librmb1, split_write_operation_1) {
  ...
  std::stringstream buffer;
  obj.set_mail_buffer(&buffer);
  obj.get_mail_buffer()->str("HALLO_WELT_");
  size_t buffer_length = obj.get_mail_buffer()->str().length();
  obj.set_mail_size(buffer_length);
...
TEST(librmb1, read_mail) {
  ...
  std::stringstream buffer;
  obj.set_mail_buffer(&buffer);
  obj.get_mail_buffer()->str("temporary temporary");
  int buffer_length = obj.get_mail_buffer()->str().length();
  obj.set_mail_size(buffer_length);
...
TEST(librmb, load_metadata) {
  ...
  std::stringstream buffer;
  obj.set_mail_buffer(&buffer);
  obj.get_mail_buffer()->str("abcdefghijklmn");
  size_t buffer_length = obj.get_mail_buffer()->str().length();
  obj.set_mail_size(buffer_length);
  ...
  obj.add_metadata(rados_metadata_1);
  key="key_2";
  val="val_2";
  const librmb::RadosMetadata rados_metadata_2=librmb::RadosMetadata::RadosMetadata(key,val);
  obj.add_metadata(rados_metadata_2);
  bool ret_storage = storage.save_mail(&obj);
...
TEST(librmb, increment_add_to_existing_key) {
  ...
  obj2.get_mail_buffer()->str("abcdefghijklmn");
  int buffer_length = obj2.get_mail_buffer()->str().length();
...
TEST(librmb, rmb_load_objects_valid_metadata) {
  ...
  obj2.get_mail_buffer()->str("hallo_welt");  // make sure obj is not empty.
  obj2.set_mail_size(obj2.get_mail_buffer()->str().length());
