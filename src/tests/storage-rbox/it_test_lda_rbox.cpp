...
TEST_F(StorageTest, mail_lda_copy_mail_in_inbox) {
  ...
  // changed: as we are copy the mail instead of moving it, that is why we need
  ...
  ASSERT_STREQ(val, val2);
  val = val2 = NULL;
  rados_utils.get_metadata(storage_interface::RBOX_METADATA_VERSION, mail2->get_metadata(), &val);
