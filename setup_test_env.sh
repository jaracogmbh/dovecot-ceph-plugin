...
docker run -d --net=ceph_config_net --name ceph_cluster_demo --mount type=tmpfs,destination=/etc/ceph -v ceph_config_test:/root/cfg -e MON_IP=192.168.204.2 -e CEPH_PUBLIC_NETWORK=192.168.204.0/24 -e CEPH_DEMO_UID=test_uuid ceph/daemon:latest-luminous demo
...
docker exec ceph_dovecot_build apt-get -qq update
...
docker exec ceph_dovecot_build sh -c 'cd /repo; ./autogen.sh && ./configure --with-dovecot=/usr/local/lib/dovecot --enable-maintainer-mode --enable-debug --with-integration-tests --enable-valgrind --enable-debug'
# docker exec ceph_dovecot_build sh -c 'cd /repo; make clean install'
